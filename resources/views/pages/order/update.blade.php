        ...
        <div class="col-md-12">
            <div class="card card-primary">
                ...
                <form action="" method="post" enctype="multipart/form-data">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <input type="hidden" name="id_bill" id="id_bill" value="{{$bill->id}}">
                            <div class="col-md-12">
                                <div class="form-group">
                                    <label for="customer_name">Tên người nhận</label>
                                    <input type="text" class="form-control @error('customer_name') is-invalid @enderror" id="customer_name" name="customer_name" placeholder="Nhập tên người nhận" value="{{$bill->customer_name}}" required>
                                    @error('customer_name')
                            ...
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="type" id="select_type" required>
                                        @if ($bill->type==0)
                                            <option value="0" selected disabled> Mua hàng offline </option>
                                        @else
                                            <option value="1" selected disabled> Mua hàng online </option>
                                        @endif
                                    </select>
                                </div>
                            </div>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <div class="input-group">
                                        <input type="number" class="form-control float-right @error('bill_phone') is-invalid @enderror" id="bill_phone" name="bill_phone" placeholder="Nhập số điện thoại" value="{{$bill->bill_phone}}" required>
                                    </div>
                                    @error('bill_phone')
                                    <small class="error-form" style="color: red;font-style: italic"> {{ $message }}</small>
                                    @enderror
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <div class="input-group">
                                        @if ($bill->type==0)
                                            <input type="text" class="form-control float-right" id="address"  name="address" placeholder="Nhập địa chỉ người nhận" value="{{$bill->address}}" disabled required>
                                        @else
                                            <input type="text" class="form-control float-right" id="address"  name="address" placeholder="Nhập địa chỉ người nhận" value="{{$bill->address}}" required>
                                        @endif
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="city_id" id="city_id">
                                        @foreach ($city as $city)
                                            <option value="{{$city->id}}" {{$city->id == $bill->city_id ? 'selected' : ''}}>{{$city->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="dist_id" id="dist_id">
                                        @foreach ($dist as $dist)
                                            <option value="{{$dist->id}}" {{$dist->id == $bill->dist_id ? 'selected' : ''}}>{{$dist->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                        ...
                        <div class="btn btn-info" onclick="location.reload()">Làm mới</div>
                        @php
                                            ...
                                            <tr>
                                                      ...
                                                      <img width="100px" src="{{$value->product_image}}" style="padding-right:10px;" alt="">
                                                        <div>
                                                            ...
                                                            @if($value->variant_id!=null)
                                                                <input type="hidden" id="last_variant_id" value="{{$value->variant_id}} {{$value->price}}">
                                                            @endif
                                                            @php
                                                                if($value->is_variation==1){
                                                                    $variant=Variantion::with('propertie')->where('product_id',$value->id)->get();
                                                                        if(isset($variant[0])){
                                                                            $html = '<select class="form-control bienthe" id="variant_id" name="variant_id[]">';
                                                ...
                                                <td>
                                                    @if($value->amount!=null)
                                                        <input type="number" name="amount[]" value="{{$value->amount}}" min="1" max="{{$value->product_quantity}}" class="form-control soluong"></td>
                                                    @else
                                                        <input type="number" name="amount[]" value="1" min="1" max="{{$value->product_quantity}}" class="form-control soluong"></td>
                                                    @endif
                                                <td class="thanhtien">
...
<script>
    function checkVariant(){
        $('.tbody-product tr').each(function() {
            div_product = $(this).find('.sanpham').children().children();
            last_variant = div_product.find('#last_variant_id').val();
            if(last_variant!=undefined){
                $('#variant_id').val(last_variant).change();
            }
        })
    }
...
@endpush
@prepend('scripts')
<script type="module" src="{{Vite::asset('resources/js/order/edit.js')}}"></script>
@endprepend
