        ...
        <div class="container-fluid">
            <div class="card card-primary row">
                <div class="card-header">
                    ...
                    @if($updateCP->updated_at!='')
                        <h3 class="card-title" style="float:right;">Cập nhật vào: {{$updateCP->updated_at}}</h3>
                    @elseif ($updateCP->created_at!='')
                        <h3 class="card-title" style="float:right;">Thêm mới vào: {{$updateCP->created_at}}</h3>
                    @else
                    @endif
                </div>
                <div class="card-body">
                    <form action="" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="row">
                            <div class="form-group col-6">
                                ...
                                <input type="text" name="category_name" class="form-control @error('category_name') is-invalid @enderror" id="category_name"  value="{{$updateCP->category_name}}">
                                @error('category_name')
                            ...
                            <div class="form-group col-12">
                                <label>Title</label>
                                <input type="text" name="seo_title" class="form-control"  value="{{$updateCP->seo_title}}" placeholder="Nội dung thẻ Meta Title dùng để SEO">
                            </div>
                            <div class="form-group col-12">
                                <label>SEO Keyword</label>
                                <input type="text" name="seo_keywords" class="form-control"  value="{{$updateCP->seo_keywords}}" placeholder="Từ khóa chính cho bài viết">
                            </div>
                            <div class="form-group col-12">
                                <label>SEO Description</label>
                                <input type="text" name="seo_description" class="form-control"  value="{{$updateCP->seo_description}}"  placeholder="Nội dung thẻ meta Description dùng để SEO ">
                            </div>
                            @csrf
                        </div>
                    </form>
