...
$("#submitAddProductSession").click(function () {
    let jsonIdProduct = [];
    // not(:disabled):
    $(":checkbox.check_id_sp:checked").each(function () {
...
let id_promotion=$('#discout_name').data('id_promotion');
dataTable(columns, url, "#dataTableLoadSpSession",{edit:id_promotion},'GET');
...
function loadMoney() {
    ...
    if (rate > 0) {
        ajaxRequest(
            ...
            function (data) {
                let arrMoney = $("#dataTableLoadSpSession tbody tr").children(
                    "td.dataMoney"
                );
                var i = 0;
