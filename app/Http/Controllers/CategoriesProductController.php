...
class CategoriesProductController extends Controller
{
    ...
    {
        $product_categories = DB::table('product_categories')->orderBy('id', 'desc')->paginate(config('define.pagination.per_page'));
        if ($search = $request->search) {
            $product_categories = DB::table('product_categories')->where('category_name', 'like', '%' . $search . '%')->orderBy('id', 'desc')->paginate(config('define.pagination.per_page'));
        }
