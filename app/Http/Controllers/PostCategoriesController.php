...
class PostCategoriesController extends Controller
{
    ...
    {
        $post_categories = DB::table('post_categories')->orderBy('id', 'desc')->paginate(config('define.pagination.per_page'));
        if ($search = $request->search) {
            $post_categories = DB::table('post_categories')->orderBy('id', 'desc')->where('category_name', 'like', '%' . $search . '%')->paginate(config('define.pagination.per_page'));
        }
